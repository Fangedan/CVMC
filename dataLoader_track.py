"""
Audiovisual Data Loader for Deep Learning Models
------------------------------------------------
This script processes and loads audiovisual data for deep learning applications, 
such as speech recognition, lip-reading, or multimodal learning.

Modules:
- os, glob: File path handling
- torch: PyTorch tensor operations
- numpy, cv2: Numerical and image processing
- random: Randomization for data augmentation
- scipy.io.wavfile: Reading audio files
- python_speech_features: Extracting MFCC features
- torchvision.transforms: Image transformations (currently unused)

Main Functions:
1. generate_audio_set(dataPath, batchList)
   - Loads and stores audio waveform data from .wav files.

2. load_audio(data, dataPath, numFrames, audioSet)
   - Extracts MFCC features from the audio, ensuring alignment with video frame rate.

3. load_visual(data, dataPath, numFrames, visualAug)
   - Loads grayscale face images from video frames and applies optional augmentations.

4. load_label(data, numFrames)
   - Extracts and formats label data from dataset files.

Classes:
- train_loader: 
  - Constructs training batches, ensuring similar-length videos are grouped together.
  - Loads audio, visual, and label features for each batch.

- val_loader:
  - Loads validation data without augmentation.
  - Extracts and returns audiovisual features and labels.
